   ...
   private static String slash = File.separator;
    ...
    public static void calculaPeajesRet(File DirEntrada, File DirSalida, int Ano,
        ...
        */
        ...
        nomLinTx = new String[numLinTx];//solo registros ìnico LÕnea#Transmisor de hoja lintron
        ...
        */
        ...
        for (int l = 0; l < numLinTx; l++) {
            ...
            if(l2==-1){//ahora la variable nomLinTx son los registros unicos en orden de lintron (asi estaba salida de prorratas)
             System.out.println();
             System.out.println("LÕnea Trocal - "+nomLineasN[l]+" - en archivo Peaje"+Ano+".xls no se encuentra en la hoja 'lintron' del archivo Ent"+Ano+".xlsx");
             System.out.println("Debe asegurarse que la LÕneas en el archivo AVI_COMA.xls sean las mismas de la hoja 'lintron' y ejecutar el botÑn Peajes");
            }
            else{
     ...
     /* ITU por cmes*/                       
        ...
        for (int j = 0; j < numClienNOExentos; j++) {
            ...
            if(l!=-1){
            ...
            else{//si no està como empresa de generaciÑn Busca si es una Distribuidora
                ...
                if(l1==-1){//si no encuentra la empresa
                    System.out.println("El Suministrador "+tmp[1]+" en 'clientes'"+" "+j+" no està asignado como Distribuidora o como empresa de GeneraciÑn");
                }
            }
        }
...
/*****************************************************************/
       ...
       GenPromMesCenO=new double[numCen][numMeses];
        ...
        for (int i = 0; i < numEmp; i++) {
            for (int j = 0; j < numTx; j++) {
                for (int k = 0; k < numMeses; k++) {
                    TotRetEmpTxO[i][j][k] = TotRetEmpTx[ne[i]][j][k];//Esto es lo que estarÕa malo
       ...
       RetEmpSinAjuTxO = new double[numEmp][numTx][numMeses];
        ...
        for (int i = 0; i < numEmp; i++) {
            for (int j = 0; j < numTx; j++) {
                for (int k = 0; k < numMeses; k++) {
                    TotRetEmpTxO[i][j][k]= re2288toNomsumi[i] == -1? TotRetEmpTxO[i][j][k]:TotRetEmpTxO[i][j][k]+TotRetEmpTxRE2288O[re2288toNomsumi[i]][j][k];//Esto es lo que estarÕa malo
        ...
        if(numClienExentos!=0){
         ...
         /*Escribe.creaH1F_2d_double(
          ...
          Escribe.crea_verificaRet(
                  ...
                  "Prorrata LÕnea",SumMensualPjeLin,
                  "Diferencia",
                  "verifica","#,###,##0;[Red]-#,###,##0;\"-\"");
